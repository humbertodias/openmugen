...
void CAirManager::AddElement(s16 nGroupNumber, s16 nImageNumber, s16 x, s16 y,
    ...
    if (!bDefaultClsn) {
        lpActionList[nTotalActionBlock - 1].AnimationElement[nTotalElement].pClnsData = (Clsn *) m_pAlloc->Alloc(
...
void CAirManager::OpenAir(char *strFileName) {
    ...
    while (!tok.AtEndOfFile()) {
        ...
        if (tok.CheckToken("[")) {
            ...
            AddAction(actionNum);
        } else if (tok.CheckToken("Clsn1Default:", false) || tok.CheckToken("Clsn2Default:", false) || tok.
                   CheckToken("Clsn1:", false) || tok.CheckToken("Clsn2:", false)) {
