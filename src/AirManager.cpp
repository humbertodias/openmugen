...
void CAirManager::RealocateActionBlock() {
    if (nTotalActionBlock > 0) {
        lpActionList[nTotalActionBlock - 1].AnimationElement =
            (Element*)m_pAlloc->Realloc(lpActionList[nTotalActionBlock - 1].AnimationElement,
                                        sizeof(Element) * lpActionList[nTotalActionBlock - 1].nNumberOfElements);
    }
}

void CAirManager::AddElement(s16 nGroupNumber, s16 nImageNumber, s16 x, s16 y, s16 nDuringTime, u16 nFlip,
                             u32 nColorFlag) {
    ...
    if (nTotalElement > nElementListSize - 1) {
        nElementListSize += 100;
        lpActionList[nTotalActionBlock - 1].AnimationElement = (Element*)m_pAlloc->Realloc(
            lpActionList[nTotalActionBlock - 1].AnimationElement, sizeof(Element) * nElementListSize);
    }
    ...
    if (!bDefaultClsn) {
        lpActionList[nTotalActionBlock - 1].AnimationElement[nTotalElement].pClnsData =
            (Clsn*)m_pAlloc->Alloc(sizeof(Clsn) * nTotalCns);

        memcpy(lpActionList[nTotalActionBlock - 1].AnimationElement[nTotalElement].pClnsData, pClsn,
               sizeof(Clsn) * nTotalCns);
...
ActionElement* CAirManager::GetAction(s32 nActionNumber) {
    for (u16 i = 0; i < nTotalActionBlock; i++) {
        if (lpActionList[i].nActionNumber == nActionNumber) return &lpActionList[i];
    }
...
void CAirManager::OpenAir(char* strFileName) {
    ...
    while (!tok.AtEndOfFile()) {
        ...
        if (tok.CheckToken("[")) {
            ...
            AddAction(actionNum);
        } else if (tok.CheckToken("Clsn1Default:", false) || tok.CheckToken("Clsn2Default:", false) ||
                   tok.CheckToken("Clsn1:", false) || tok.CheckToken("Clsn2:", false)) {
            ...
            for (int c = 0; c < clsnBoxCount; c++) {
                if (!(tok.CheckToken("Clsn1") || tok.CheckToken("Clsn2")))
                // for some reason kfm uses clsn2 sometimes in clsn1 items
                {
                }
