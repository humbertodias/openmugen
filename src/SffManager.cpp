...
bool CSffManager::LoadActToSff(const char* strActFile) {
    ...
    if (Act == NULL) PrintMessage("CSffSpriteManager %s File not found", strActFile);
    ...
    for (int j = 255; j >= 0; j--) {
        ...
        ColorPallet[j] = m_pVideoSystem->MapRGB(r, g, b);
    }
...
void CSffManager::BlitSprite(s16 nGroupNumber, s16 nImageNumber, s16 x, s16 y) {
    ...
    if (nFlags == BLT_NORMAL) m_pVideoSystem->NormalBlt(&lpSpriteList[i], x, y, false);

    if (nFlags == BLT_NORMALMASKED) m_pVideoSystem->NormalBlt(&lpSpriteList[i], x, y, true);

    if (nFlags == BLT_FLIPH) m_pVideoSystem->NormalFlipH(&lpSpriteList[i], x, y, false);

    if (nFlags == BLT_FLIPHMASKED) m_pVideoSystem->NormalFlipH(&lpSpriteList[i], x, y, true);
}
...
void CSffManager::BlitAnim(s16 x, s16 y) {
    BlitSprite(Anim->AnimationElement[Anim->nCurrentImage].nGroupNumber,
               Anim->AnimationElement[Anim->nCurrentImage].nImageNumber, x, y);
    ...
    if (Anim->nDuringTime <= m_pTimer->GetGameTime() && Anim->AnimationElement[Anim->nCurrentImage].nDuringTime != -1) {
        ...
        if (Anim->nCurrentImage >= Anim->nNumberOfElements) {
            ...
            if (Anim->loopStart != -1) Anim->nCurrentImage = Anim->loopStart;
        }
...
u8* CSffManager::DecodePcx(u8* PcxByte, PCXHEADER header) {
    ...
    u32 nCountByte, nTotalyByte, nHowManyBytes, nHowManyBlank;
    ...
    if (widht < header.bytesPerLine * header.NPlanes) widht = header.bytesPerLine * header.NPlanes;
        ...
        while (x < widht) {
            ...
            while (size-- > 0) {
                ...
                if (x == widht && widht != header.widht) {
                    ...
                    for (u32 i = 0; i < nHowManyBlank; i++) Pos += 2;
                }
                x++;
            }
        }
...
void CSffManager::DecodeSffFile() {
    ...
    while (!feof(lpSffFile)) {
        ...
        if (subheader.LenghtOfSubheader != 0) {
            ...
            lpSpriteList[nTotalImages].PcxHeader.widht =
                lpSpriteList[nTotalImages].PcxHeader.widht - lpSpriteList[nTotalImages].PcxHeader.x + 1;
            lpSpriteList[nTotalImages].PcxHeader.height =
                lpSpriteList[nTotalImages].PcxHeader.height - lpSpriteList[nTotalImages].PcxHeader.y + 1;
            ...
            lpSpriteList[nTotalImages].byPcxFile = DecodePcx(tempByte, lpSpriteList[nTotalImages].PcxHeader);
            ...
            if (fgetc(lpSffFile) == 12 && !subheader.PalletSame && !bPalletLoaded &&
                lpSpriteList[nTotalImages].PcxHeader.NPlanes <= 1) {
                for (int j = 0; j < 256; j++) {
                    int r, g, b;
                    r              = fgetc(lpSffFile);
                    g              = fgetc(lpSffFile);
            ...
            memcpy(&lpSpriteList[nTotalImages], &lpSpriteList[subheader.IndexOfPrevious - 1], sizeof(SFFSPRITE));
...
int CSffManager::FindSprite(s16 nGroupNumber, s16 nImageNumber) {
    for (int i = 0; i < nImageListSize; i++) {
        if (lpSpriteList[i].GroupNumber == nGroupNumber && lpSpriteList[i].ImageNumber == nImageNumber) {
            return i;
        }
    }
